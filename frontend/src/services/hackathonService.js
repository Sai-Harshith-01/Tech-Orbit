...
export const hackathonService = {
  ...
  registerForHackathon: async (hackathonId, data) => {
    if (data) {
      const response = await api.post(`/hackathons/${hackathonId}/register`, data, {
  ...
  verifyRegistration: async (registrationId, status) => {
    const formData = new FormData();
    formData.append('approve', (status === 'APPROVED').toString());
    const response = await api.post(`/college/registrations/${registrationId}/verify`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
    return response.data;
  },
