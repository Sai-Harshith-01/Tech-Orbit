  ...
  const fetchData = async () => {
    try {
      const [hData, rData] = await Promise.all([
        ...
        hackathonService.getMyRegistrations()
      ]);
      setHackathons(hData);
      setMyRegistrations(rData);
    } catch (error) {
   ...
   <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 animate-zoomIn">
    ...
    <header className="mb-10">
     <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">
      🏆 Available <span className="text-brand-600">Events</span>
     </h1>
      ...
      {hackathons.length > 0 ? (
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {hackathons.map((hackathon) => (
         <HackathonCard
          key={hackathon.id}
          hackathon={hackathon}
          registration={myRegistrations.find(r => r.hackathon_id === hackathon.id)}
       ...
       <div className="premium-card py-20 text-center text-slate-500 flex flex-col items-center">
        <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center text-3xl mb-4">
         ...
         <p className="text-xl font-bold text-slate-700">No events live yet</p>
