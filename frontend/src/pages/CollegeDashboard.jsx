      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 animate-zoomIn">
        ...
        {loading ? (
          ...
          <div className="space-y-12">
            ...
            {hackathons.some(h => h.pending_verifications > 0) && (
              <div className="bg-amber-50 border-2 border-amber-200 rounded-3xl p-6 shadow-lg shadow-amber-200/20 animate-pulse-subtle">
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-amber-900">Pending <span className="text-amber-600">Verifications</span></h3>
                      <p className="text-amber-700/80 text-sm font-medium">You have students waiting for payment approval across your events.</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    {hackathons.filter(h => h.pending_verifications > 0).map(h => (
                      <button 
                        key={h.id}
                        onClick={() => navigate(`/college/hackathons/${h.id}/participants`)}
                        className="bg-amber-500 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md shadow-amber-500/30 hover:bg-amber-600 transition-all flex items-center gap-2 whitespace-nowrap"
                      >
                        Verify {h.hackathon_name} ({h.pending_verifications})
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {hackathons.length > 0 ? (
                      ...
                      <div className="p-6 flex flex-col flex-1">
                        ...
                        <div className="grid grid-cols-1 gap-3 mt-auto">
                          <Link
                            ...
                            className="relative bg-brand-50 text-brand-600 py-2.5 rounded-premium text-sm font-bold text-center hover:bg-brand-100 transition-all flex items-center justify-center gap-2"
                          >
                            👁️ View Roster
                            {hackathon.pending_verifications > 0 && (
                              <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full shadow-lg animate-bounce border-2 border-white">
                                {hackathon.pending_verifications}
                              </span>
                            )}
                          </Link>
                          <button
                            onClick={() => handleDelete(hackathon.id, hackathon.hackathon_name)}
                            className="bg-red-50 text-red-600 py-2.5 rounded-premium text-sm font-bold text-center hover:bg-red-100 transition-all flex items-center justify-center gap-2 border border-red-100"
                          >
                            🗑️ Delete Event
                          </button>
                        </div>
                ...
                <div className="premium-card py-24 text-center text-slate-500 flex flex-col items-center border-dashed border-2">
                  ...
                  <p className="text-slate-500 max-w-sm px-4">
                    Ready to discover talent? Create an event to start receiving registrations from students.
                  </p>
                  <button
                    onClick={() => navigate('/college/create-hackathon')}
